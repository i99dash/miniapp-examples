/**
 * WebView Doctor — car WebView / bridge capability analyzer.
 *
 * The fallback diagnostic: a user opens this when another mini-app
 * doesn't work, exports a .txt report, and sends it to i99dash
 * support on Telegram.
 *
 * Design rule that differs from every other mini-app: the usual
 * "host not ready → show a notice and bail" pattern is exactly the
 * failure this tool exists to diagnose — so here the *wrongness IS
 * the report*. It MUST render a usable report under no-host, the
 * oldest WebView, a thrown JS error, or a dead bridge. Therefore it
 * is hand-written to an ES5-safe runtime floor: no bare `globalThis`
 * identifier (ES2020 — esbuild es2019 down-levels syntax but NOT
 * this global), no ES2020+ builtins in its own code path (it only
 * *probes* their existence), nothing that can itself blank.
 *
 * Privacy: NO VIN, NO location, NO account/user id values, NO signal
 * values — only technical/timing/capability facts. The report is
 * meant to be safely pasted to support.
 */
(function () {
  'use strict';

  // ── 0. Error capture FIRST ────────────────────────────────────────
  // Installed before anything else so a failure anywhere downstream
  // is recorded into the report instead of blanking the screen.
  var ERRORS = [];
  ...
  // ── 1. Safe global ────────────────────────────────────────────────
  // `typeof` on an undeclared name never throws, so this probe is
  // itself safe even where `globalThis` (ES2020) is undefined.
  var G =
  ...
  // ── 2. Host resolution (same order as _shared/l5compat.js) ────────
  ...
  var DIAG = {
    ...
    host: {
      readyAtSync: false,
      readyVia: null, // 'sync' | 'event' | 'poll' | 'timeout(null)' | 'timeout(late)'
      readyMs: null,
      which: null, // '__i99dashHost' | 'flutter_inappwebview' | null
      eventHub: null,
    },
    webview: {},
    features: {},
  ...
  // Which global the host arrived on (diagnostic — branded vs legacy).
  function hostWhich() {
    try {
  ...
  // Instrumented readiness wait — the single most important signal.
  function whenHostReady(timeoutMs) {
    ...
    return new Promise(function (resolve) {
      ...
      var onReady = function () { tryNow('event'); };
      try {
        if (w && w.addEventListener)
          w.addEventListener('flutterInAppWebViewPlatformReady', onReady);
      } catch (_) {}
      poll = setInterval(function () { tryNow('poll'); }, 150);
  ...
  // ── 3. WebView + feature probes (pure, no host needed) ────────────
  function probeWebView() {
    var wv = {};
    try { wv.userAgent = String(navigator.userAgent || ''); } catch (_) { wv.userAgent = '?'; }
    var m;
    try { m = /Chrom(?:e|ium)\/(\d+)/.exec(wv.userAgent); } catch (_) {}
    wv.chromiumMajor = m ? parseInt(m[1], 10) : null;
    try { var am = /Android\s+([0-9.]+)/.exec(wv.userAgent); wv.android = am ? am[1] : null; } catch (_) {}
    try { wv.dpr = G.devicePixelRatio || null; } catch (_) {}
    try { wv.screen = (screen.width || '?') + 'x' + (screen.height || '?'); } catch (_) {}
    try { wv.inner = (window.innerWidth || '?') + 'x' + (window.innerHeight || '?'); } catch (_) {}
    try { wv.lang = (navigator.language || ''); } catch (_) {}
    DIAG.webview = wv;
  }
  ...
  function probeFeatures() {
    var f = {};
    // ES2020+ identifiers/builtins — the Gate-B-relevant ones.
    f.globalThis = typeof globalThis !== 'undefined';
    f.optionalChaining_es2020 = null; // syntax — not runtime-detectable here
    f.structuredClone = has(function () { return typeof G.structuredClone === 'function'; });
    f.Promise_allSettled = has(function () { return typeof Promise !== 'undefined' && typeof Promise.allSettled === 'function'; });
    f.Promise_any = has(function () { return typeof Promise.any === 'function'; });
    f.Array_at = has(function () { return typeof [].at === 'function'; });
    f.String_replaceAll = has(function () { return typeof ''.replaceAll === 'function'; });
    f.Object_fromEntries = has(function () { return typeof Object.fromEntries === 'function'; });
    f.BigInt = has(function () { return typeof G.BigInt === 'function'; });
    f.crypto_randomUUID = has(function () { return G.crypto && typeof G.crypto.randomUUID === 'function'; });
    // APIs this report's export path relies on.
    f.Blob = has(function () { return typeof G.Blob === 'function'; });
    f.URL_createObjectURL = has(function () { return G.URL && typeof G.URL.createObjectURL === 'function'; });
    f.navigator_clipboard = has(function () { return navigator.clipboard && typeof navigator.clipboard.writeText === 'function'; });
    f.execCommand = has(function () { return typeof document.execCommand === 'function'; });
    f.download_attr = has(function () { return 'download' in document.createElement('a'); });
    f.localStorage = has(function () { var k = '__wd'; localStorage.setItem(k, '1'); localStorage.removeItem(k); return true; });
    f.fetch = has(function () { return typeof G.fetch === 'function'; });
    f.addEventListener = has(function () { return typeof window.addEventListener === 'function'; });
    DIAG.features = f;
  }

  // ── 4. Bridge probes (status/shape/latency only — NO values, NO PII)
  function callHost(host, handler, payload) {
    // Mirrors _shared/l5compat.js `call()` — flat payload, returns
    // the result directly.
  ...
  function timed(label, p) {
    var s = now();
    return p.then(
      function (r) { return { label: label, ok: true, ms: Math.round(now() - s), r: r }; },
      function (e) { return { label: label, ok: false, ms: Math.round(now() - s), err: (e && e.message) || String(e) }; }
    );
  }

  function probeBridge(host) {
    DIAG.host.eventHub =
      G.__i99dashEvents && typeof G.__i99dashEvents.on === 'function'
        ? 'present'
        : 'absent';
    if (!host) {
    ...
    return Promise.all(
      jobs.map(function (j) {
        return j.then(function (x) { return x; });
      })
    ).then(function (res) {
      ...
      for (var i = 0; i < res.length; i++) {
        ...
        if (x.label === 'getContext' && x.ok && x.r && typeof x.r === 'object') {
          ...
          entry.hasUserId = !!x.r.userId; // value intentionally NOT recorded
          entry.hasActiveCarId = !!x.r.activeCarId; // VIN-ish — NOT recorded
        }
        if (x.label === 'car.read' && x.ok) {
          var vals = x.r && x.r.values;
          entry.isErrorEnvelope = !!(x.r && x.r.error);
          entry.valueCount =
            vals && typeof vals === 'object' ? Object.keys(vals).length : 0;
        }
  ...
  // ── 5. Verdict (most-severe-applicable) ───────────────────────────
  ...
  // ── 6. Report text (compact summary + sections + JSON) ────────────
  ...
  function buildReport(verdict) {
    ...
    var json;
    try { json = JSON.stringify(DIAG); } catch (e) { json = '{"jsonError":"' + ((e && e.message) || e) + '"}'; }
    L.push(json);
    L.push('===== end =====');
    return L.join('\n');
  }

  // ── 7. Render + export (.txt file primary, textarea fallback) ─────
  function $(id) { return document.getElementById(id); }
  function setStatus(s) { try { $('status').textContent = s; } catch (_) {} }

  function render() {
    var verdict;
    try { verdict = computeVerdict(); } catch (e) { recordError('verdict', e); verdict = { cls: 'bad', text: 'verdict failed: ' + ((e && e.message) || e) }; }
    var text;
    try { text = buildReport(verdict); } catch (e) { recordError('report', e); text = 'Report build failed: ' + ((e && e.message) || e) + '\nerrors=' + ERRORS.join(' || '); }
    ...
    try { $('report').value = text; } catch (_) {}

    var fname =
      'webview-doctor-' +
      (DIAG.webview.chromiumMajor != null ? 'cr' + DIAG.webview.chromiumMajor + '-' : '') +
      new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19) +
      '.txt';

    try {
      $('dl').onclick = function () { exportTxt(text, fname); };
      $('cp').onclick = function () { copyText(text); };
    } catch (_) {}
  }
  ...
  function exportTxt(text, fname) {
    // Primary: real downloadable file (user attaches it in Telegram).
    try {
    ...
    } catch (e) { recordError('export.blob', e); }
    ...
    try {
      ...
      a2.click();
      setTimeout(function () { try { document.body.removeChild(a2); } catch (_) {} }, 1500);
      setStatus('exported ' + fname + ' (data-uri) — attach it in Telegram');
      return;
    } catch (e2) { recordError('export.datauri', e2); }
    // Last resort: this WebView blocks downloads — tell the user to
    // select-all the box and paste into Telegram instead.
    try { $('report').focus(); $('report').select(); } catch (_) {}
    ...
    try {
      if (navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
        navigator.clipboard.writeText(text).then(
          function () { setStatus('copied — paste into Telegram'); },
  ...
  // ── 8. Run — ALWAYS ends with a rendered report ───────────────────
  function run() {
    try { probeWebView(); } catch (e) { recordError('probeWebView', e); }
    try { probeFeatures(); } catch (e) { recordError('probeFeatures', e); }
    // Render an immediate partial report so the user sees output even
    // if readiness/bridge hangs.
    render();
    setStatus('waiting for host…');

    whenHostReady(8000).then(
      function (host) {
  ...
  // Top-level guard: even a catastrophic failure renders a report.
  try {
